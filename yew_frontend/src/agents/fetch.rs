            ...
            Msg::Data(who, fetch_response) => match fetch_response {
                FetchResponse::Error(error) => {
                    match error {
                        ...
                        | FetchError::AuthAbsent => self.router.send(RouteRequest::ChangeRoute(RouteInfo::from("/auth/login"))),
                        _ => {}
                    }
...
fn refresh_jwt(jwt_string: &str, fetch_service: &mut FetchService, fetch_task_collection: &mut Vec<FetchTask>) {
    ...
    let closure = move |response: Response<Result<String, Error>>| {
        ...
        if meta.status.is_success() {
            ...
            let callback = Callback::from(|_: RouteInfo| ());// NOOP
            let mut router = RouteAgentBridge::new(callback);
            let route = RouteInfo::from("/auth/login");
...
use yew_router::agent::RouteSenderBridge;
use uuid::Uuid;
...
impl Networking {
    pub fn new<T>(link: &mut ComponentLink<T>) -> Self
        where
    ...
    pub fn fetch<T, U, V>(&mut self, request: &T, response_mapper: fn(FetchResponse<U>) -> V::Message, link: &mut ComponentLink<V>)
        where
            T: FetchRequest,
        ...
        let handle_response_closure = move |response: Response<Text>| {
            ...
            let message: FetchResponse<U> = match meta.status.into() {
                401 => {
                    ...
                    let mut router = RouteSenderBridge::new(router_cb.clone());
                    router.send(RouteRequest::ChangeRoute(RouteInfo::from("/auth/login")));
    ...
    pub fn fetch_string<T, V>(&mut self, request: &T, response_mapper: fn(FetchResponse<String>) -> V::Message, link: &mut ComponentLink<V>)
        where
            T: FetchRequest,
            V: Component + Renderable<V>,
            V::Message: Default
...
fn make_request<T: FetchRequest>(fetch_struct: &mut Networking, request: &T, callback: Callback<Response<Text>>) -> Option<FetchTask> {
    ...
    if let Some(token) = user::get_token_if_valid(&mut fetch_struct.storage_service) {
        ...
        fetch_struct.router.send(RouteRequest::ChangeRoute(RouteInfo::from("/auth/login")));
