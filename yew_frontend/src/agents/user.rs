...
pub fn extract_payload_from_jwt(jwt_string: &str) -> Result<JwtPayload<Uuid>, failure::Error> {
    let payload_segment: &str = jwt_string
        .split('.')
        .collect::<Vec<&str>>()
        .get(1)
        .ok_or_else(|| failure::Error::from(JwtError::UnexpectedNumberOfSections))?;
    let payload_json: JsonValue = decode_payload(payload_segment)
        .map_err(|_| failure::Error::from(JwtError::Base64DecodeFailure))?;
    serde_json::from_value(payload_json)
        .map_err(|_| failure::Error::from(JwtError::JsonDecodeFailure))
}

/// Decodes the payload section of the token using a base 64 decoder.
///
/// Taken from frank_jwt source
fn decode_payload(payload_segment: &str) -> Result<JsonValue, failure::Error> {
...
pub fn user_id(storage: &mut StorageService) -> Result<Uuid, failure::Error> {
...
pub fn user_auth_expire_date(storage_service: &mut StorageService) -> Result<NaiveDateTime, failure::Error> {
...
pub fn store_jwt(storage_service: &mut StorageService, jwt: String) {
    let jwt: Result<String, failure::Error> = Ok(jwt);
    storage_service.store("JWT", jwt)
}

pub fn restore_jwt(storage_service: &mut StorageService) -> Result<String, failure::Error> {
