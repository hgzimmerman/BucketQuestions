...
use authorization::{Secret, BEARER, AUTHORIZATION_HEADER_KEY};
...
use db::bucket::db_types::{NewBucket, Bucket, BucketUserRelation, BucketUserPermissions};
use crate::api::auth::test::get_jwt;
use warp::http::status::StatusCode;
use db::test::bucket_and_user_fixture::BucketAndUserFixture;
use crate::error::ErrorResponse;
use crate::api::bucket::{SetPermissionsRequest, ChangeBucketFlagsRequest};
...
#[test]
fn create_bucket() {
    execute_test_on_repository(|_fix: &UserBucketRelationFixture, provider: RepositoryProvider| {
...
#[test]
fn get_bucket() {
    execute_test_on_repository(|fix: &UserBucketRelationFixture, provider: RepositoryProvider| {
        let state = State::testing_init(provider, Secret::new("hello"));
        let filter = routes(&state);
...
fn get_permissions_for_self() {
    execute_test_on_repository(|fix: &UserBucketRelationFixture, provider: RepositoryProvider| {
        ...
        assert_eq!(res.set_exclusive_permission, fix.relation.set_exclusive_permission);
        assert_eq!(res.set_public_permission, fix.relation.set_public_permission);
        assert_eq!(res.set_drawing_permission, fix.relation.set_drawing_permission);
    });
}
