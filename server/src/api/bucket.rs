...
pub fn bucket_api(state: &State) -> impl Filter<Extract=(impl Reply,), Error=Rejection> + Clone{
    ...
    let get_bucket_by_uuid = path!(Uuid)
        .and(warp::get2())
        .and(state.db())
        .map(|uuid: Uuid, conn: PooledConn| -> Result<Bucket, Error> {
    ...
    let get_buckets_user_is_in = path!("in")
        ...
        .and(user_filter(state))
        .and(state.db())
        .map(|user_uuid: Uuid, conn: PooledConn| -> Result<Vec<Bucket>, Error> {
    ...
    let get_permissions_for_self = path!( Uuid / "user")
        ...
        .and(user_filter(state))
        .and(state.db())
        .map(|bucket_uuid: Uuid, user_uuid: Uuid, conn: PooledConn| {
    ...
    let set_permissions = path!(Uuid / "user")
        ...
        .and(user_filter(state))
        .and(state.db())
        .map(|bucket_uuid: Uuid, permissions_request: SetPermissionsRequest, user_uuid: Uuid, conn: PooledConn| -> Result<BucketUserJoin, Error> {
    ...
    let set_bucket_drawing = path!(Uuid)
        ...
        .and(user_filter(state))
        .and(state.db())
        .map(|bucket_uuid: Uuid, request: ChangeDrawingRequest, user_uuid: Uuid, conn: PooledConn| {
            let permissions_for_acting_user = conn.get_permissions(user_uuid, bucket_uuid).map_err(Error::from)?;
    ...
    let set_bucket_visibility = path!(Uuid)
        ...
        .and(user_filter(state))
        .and(state.db())
        .map(|bucket_uuid: Uuid, request: ChangeVisibilityRequest, user_uuid: Uuid, conn: PooledConn| {
            let permissions_for_acting_user = conn.get_permissions(user_uuid, bucket_uuid).map_err(Error::from)?;
            if permissions_for_acting_user.set_drawing_permission {
